...
def addSuburb(this, localityPid, statePid, suburb, alias, sa1, lga, latitude, longitude):
    ...
    if alias == 'P':
        ...
        if 'A' not in suburbs[soundCode][suburb][statePid]:
            suburbs[soundCode][suburb][statePid]['A'] = {}
        suburbs[soundCode][suburb][statePid]['A'][localityPid] = [sa1, lga, latitude, longitude]
    elif alias == 'C':
...
def initData(this):
    ...
    for soundCode in suburbs:
        for suburb in suburbs[soundCode]:
            for statePid in suburbs[soundCode][suburb]:
                if statePid == 'SX':
                    continue
                srcs = suburbs[soundCode][suburb][statePid]
                for src in ['G', 'GA']:
                    if src in srcs:
            ...
            if len(this.validSuburbs) > 0:
                ...
                for thisSuburb in this.validSuburbs:
                    ...
                    if len(suburbs[soundCode][thisSuburb]) == 1:
                        ...
                        this.logger.debug('Rules1and2 - suburbs named %s in multiple states', thisSuburb)
