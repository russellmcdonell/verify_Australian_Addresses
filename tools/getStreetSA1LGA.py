...
SYNOPSIS
$ python getStreetSA1LGA.py [-G GNAFdir|--GNAFdir=GNAFdir] [-A ABSdir|--ABSdir=ABSdir]
                            [StreetSA1LGAoutputFile] [-v loggingLevel|--verbose=logingLevel]
...
This script read the SA1 and LGA polygons and then reads the G-NAF psv files of geocode street locality information (*_STREET_LOCALITY_POINT_psv.psv)
Each G-NAF street_locality_pid has a latitude and longitude. This script finds the SA1 and LGA polygons that bound this point.
'''
...
def findPolygon(shapes, records, st_pid, long, lat):
    '''
...
if __name__ == '__main__':
    ...
    Then reads the G-NAF psv file (*_STREET_LOCALITY_POINT_psv.psv)
    Each G-NAF street_locality_pid has a latitude and longitude.
    ...
    parser.add_argument('-G', '--GNAFdir', dest='GNAFdir', default='../G-NAF',
                        help='The name of directory containing the G-NAF psv files (default ../G-NAF)')
    ...
    parser.add_argument ('StreetSA1LGAoutputFile', nargs='?', default='street_SA1LGA.psv',
    ...
    for SandT in ['ACT', 'NSW', 'NT', 'OT', 'QLD', 'SA', 'TAS', 'VIC', 'WA']:
        ...
        with open(streetLocalityFile, 'rt', newline='', encoding='utf-8') as streetFile:
            ...
            for row in streetReader:
                ...
                SA1 = findPolygon(SA1shapes, SA1records, street_pid, longitude, latitude)
                if SA1 is None:
                    logging.warning('street_pid(%s)[%.7f,%.7f] is not inside any SA1 polygon - looking for nearest polygon',
                                    street_pid, latitude, longitude)
                    SA1 = findNearestPolygon(SA1shapes, SA1records, longitude, latitude)
                if SA1 is None:
                    logging.warning('street_pid(%s)[%s,%s] is not inside any SA1 polygon bounding box',
                                    street_pid, latCode, longCode)
                LGA = findPolygon(LGAshapes, LGArecords, street_pid, longitude, latitude)
                if LGA is None:
                    logging.warning('street_pid(%s)[%.7f,%.7f] is not inside any LGA polygon - looking for nearest polygon',
                                    street_pid, latitude, longitude)
                    LGA = findNearestPolygon(LGAshapes, LGArecords, longitude, latitude)
                if LGA is None:
                    logging.warning('street_pid(%s)[%s,%s] is not inside any LGA polygon bounding box',
                                    street_pid, latCode, longCode)
                if (SA1 is not None) or (LGA is not None):
                    logging.debug('Found street_pid(%s)[%s,%s], SA1(%s), LGA(%s)', street_pid, longCode, latCode, SA1, LGA)
                    outRow = [street_pid, SA1, LGA, longitude, latitude]
                    streetSA1LGAwriter.writerow(outRow)
